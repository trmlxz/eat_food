...
家长菜单“更多设置”可选择哭泣（带眼泪）、微笑、大笑三种表情，默认微笑，支持实时预览与保存；同时提供胃部蠕动开关，以及独立的速度、幅度 1～5 级调节，附实时预览。默认关闭、速度 3 级、幅度 2 级；保存后作用于扫描结果并在刷新后保留。速度从每周期 6 秒到 1.2 秒，幅度从轻微到明显。旧版本设置自动补全默认值。系统开启减少动态效果时暂停蠕动。
